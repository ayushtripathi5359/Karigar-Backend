"""Address CRUD service. Encrypts line1/line2 explicitly via SQL."""
...
async def create_address(
    ...
    *,
    ...
    if make_default:
        # Clear the existing default first — partial unique index on (user_id) WHERE is_default
        # would otherwise reject the new row.
        await session.execute(
            text(
        ...
        new_id = (
            await session.execute(
                text(
                    ...
                    INSERT INTO addresses (
                        user_id, label,
                        line1_encrypted, line2_encrypted,
                        city, state, pincode, country, is_default
                    ) VALUES (
                        :u, :label,
                        encrypt_pii(:line1), encrypt_pii(:line2),
                        :city, :state, :pincode, :country, :is_default
                    )
                    RETURNING address_id
                    """
...
async def set_default(
    ...
    # Make sure the address belongs to this user (RLS would block anyway but
    # surface a 404 instead of a silent no-op for clarity).
