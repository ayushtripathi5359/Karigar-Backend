"""Owns the order row + items + status transitions."""
...
async def create_draft(
    ...
    *,
    ...
    """Create a 'draft' order with one line item and totals computed from the stone price."""
    if quantity < 1:
        raise HTTPException(status.HTTP_400_BAD_REQUEST, "quantity must be >= 1")

    # Fetch the stone price (under the order_items semantics: unit_price = sm.price).
    row = (
    ...
    # No tax/discount logic yet; total = subtotal
    total = subtotal
    ...
    # Single line item — total_price_inr is GENERATED in the DB, do NOT write to it.
    await session.execute(
        text(
            """
...
async def transition(
    ...
    *,
    ...
    """Move an order to a new status. RLS scopes the update to the buyer."""
...
async def get_order(
    ...
    """Order detail joined with its single item + tracking."""
    order = (
        await session.execute(
            text(
                """
