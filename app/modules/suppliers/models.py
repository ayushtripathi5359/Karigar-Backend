"""ORM mappings for suppliers, supplier_uploads, scrape_jobs. SCAFFOLD."""
...
class ScrapeJob(Base):
    ...
    scrape_job_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, server_default=func.gen_random_uuid())
    source_url: Mapped[str] = mapped_column(String, nullable=False)
    status: Mapped[str] = mapped_column(String, nullable=False, default="queued")
    rows_scraped: Mapped[int] = mapped_column(Integer, nullable=False, default=0)
    error_log: Mapped[dict | None] = mapped_column(JSONB, nullable=True)
