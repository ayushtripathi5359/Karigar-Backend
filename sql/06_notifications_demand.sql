-- =============================================================================
-- KARIGAR — Notification, push outbox, campaigns, and demand requests.
-- Apply after sql/05_safe_decrypt_pii.sql for non-Alembic local setup.
-- =============================================================================

ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'transaction';
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'supplier_upload';
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'supplier_update';
ALTER TYPE notification_type ADD VALUE IF NOT EXISTS 'demand_request';
...
DROP POLICY IF EXISTS device_push_tokens_delivery_read ON device_push_tokens;
DROP POLICY IF EXISTS device_push_tokens_owner_insert ON device_push_tokens;
DROP POLICY IF EXISTS device_push_tokens_owner_update ON device_push_tokens;
DROP POLICY IF EXISTS device_push_tokens_owner_delete ON device_push_tokens;
DROP POLICY IF EXISTS notification_outbox_admin_read ON notification_outbox;
DROP POLICY IF EXISTS notification_outbox_service_insert ON notification_outbox;
DROP POLICY IF EXISTS notification_outbox_admin_update ON notification_outbox;
DROP POLICY IF EXISTS notification_campaigns_admin ON notification_campaigns;
DROP POLICY IF EXISTS demand_requests_scope ON demand_requests;
DROP POLICY IF EXISTS users_notification_recipient_read ON users;
