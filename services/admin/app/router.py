...
from karigar_shared.auth.deps import CurrentUser, require_roles
...
async def export_users(
    ...
    rows = (
        await me.session.execute(
            text(
                f"""
                SELECT user_id, email, safe_decrypt_pii(phone_encrypted) AS phone,
                       safe_decrypt_pii(full_name_encrypted) AS full_name,
    ...
    return _csv_response(
        "admin-users.csv",
        ["user_id", "email", "phone", "full_name", "role", "is_verified", "created_at", "last_login_at", "deleted_at"],
        rows,
    )
...
async def export_suppliers(
    ...
    rows = (
        await me.session.execute(
            text(
                f"""
                SELECT supplier_id, supplier_code, display_name, contact_email,
                       contact_phone, tier, is_active, onboarded_at, created_at, updated_at
                FROM suppliers
                WHERE {' AND '.join(where)}
                ORDER BY display_name
                """
    ...
    return _csv_response(
        "admin-suppliers.csv",
        ["supplier_id", "supplier_code", "display_name", "contact_email", "contact_phone", "tier", "is_active", "onboarded_at", "created_at", "updated_at"],
        rows,
    )
