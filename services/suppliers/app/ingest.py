...
async def append_upload_errors(
    db: AsyncSession,
    *,
    upload_id: UUID,
    errors: list[dict],
    ...
    await db.execute(
        text(
            """
            UPDATE supplier_uploads SET
                rows_total = COALESCE(rows_total, 0) + :total,
                rows_failed = COALESCE(rows_failed, 0) + :failed,
                error_log = COALESCE(error_log, '[]'::jsonb) || CAST(:errors AS jsonb),
                status = CASE
                           WHEN COALESCE(rows_imported, 0) > 0 THEN 'partial'::upload_status
                           ELSE 'failed'::upload_status
                         END,
                processed_at = now()
            WHERE upload_id = :upload_id
