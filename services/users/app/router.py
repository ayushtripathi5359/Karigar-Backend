...
from app.schemas import (
    AddressCreateBody, AddressListResponse, AddressResponse,
    OnboardingCompleteBody, OnboardingCompleteResponse, UserProfileResponse,
)
...
async def complete_onboarding(
    ...
    completed_at = await profile_svc.complete_onboarding(
        me.session, user_id=me.user_id, display_name=body.display_name, email=body.email
    )
    return OnboardingCompleteResponse(onboarding_completed=True, completed_at=completed_at)


@router.get("/users/me/addresses", response_model=AddressListResponse, summary="List my addresses")
@limiter.limit("60/minute")
...
async def create_address(
    ...
    row = await addr_svc.create_address(
        me.session, user_id=me.user_id, label=body.label, line1=body.line1,
        line2=body.line2, city=body.city, state=body.state, pincode=body.pincode,
        country=body.country, make_default=body.make_default,
    )
    return AddressResponse(**row)


@router.post("/users/me/addresses/{address_id}/set-default", response_model=AddressResponse,
...
@router.delete("/users/me/addresses/{address_id}", status_code=204, summary="Soft-delete an address")
@limiter.limit("30/minute")
